...
개발 중에는 Vite dev server로 React 화면을 확인할 수 있습니다. Chrome Extension 동작 검증은 빌드 후 `dist` 폴더를 Chrome에서 로드해서 확인합니다.
...
이 명령은 빌드를 실행한 뒤 `focus-guard.zip` 파일을 생성합니다. Chrome Web Store 등록 시 zip 파일을 업로드할 수 있습니다.
